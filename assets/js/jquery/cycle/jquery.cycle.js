...
(function($){
	var l="2.73";
	if(!$.support==undefined && $.browser){
		$.support={opacity:!($.browser.msie)}
	}
	function a(q){
		if($.fn.cycle.debug){f(q)}
	}
	function f(){
	...
	$.fn.cycle=function(r,q){
		...
		if(this.length===0&&r!="stop"){
			...
			f("terminating; zero elements found by selector"+($.isReady?"":" (DOM not ready)"));
			return this
		}
		return this.each(function(){
			...
			var x=$(this);
			var y=w.slideExpr?$(w.slideExpr,this):x.children();
			var u=y.get();
			if(u.length<2){
	...
	function m(q,t,r){
		...
		if(t.constructor==String){
			switch(t){
				case"stop":
					q.cycleStop++;
					if(q.cycleTimeout){clearTimeout(q.cycleTimeout)}
					q.cycleTimeout=0;
					$(q).removeData("cycle.opts");
					return false;
				case"pause":
					q.cyclePause=1;
					return false;
			...
			if(t.constructor==Number){
				...
				if(typeof r=="string"){t.oneTimeFx=r}
	...
	function b(q,r){
		if(!$.support.opacity&&r.cleartype&&q.style.filter){
			try{
	...
	function k(y,J,u,t,E){
		var C=$.extend({},$.fn.cycle.defaults,t||{},$.metadata?y.metadata():$.meta?y.data():{});
		if(C.autostop){
		...
		if(!$.support.opacity&&C.cleartype){
			C.after.push(function(){
				b(this,C)
			})
		}
		if(C.continuous){
			C.after.push(function(){e(u,C,0,!C.rev)})
		}
		n(C);
		if(!$.support.opacity&&C.cleartype&&!C.cleartypeNoBg){g(J)}
		...
		if(C.random){
			...
			C.randomMap.sort(function(L,w){return Math.random()-0.5});
			C.randomIndex=0;
			C.startingSlide=C.randomMap[0]
		}else{
			if(C.startingSlide>=u.length){C.startingSlide=0}
		}
		...
		$(u[x]).css("opacity",1).show();
		b(u[x],C);
		if(C.fit&&C.width){J.width(C.width)}
		if(C.fit&&C.height&&C.height!="auto"){J.height(C.height)}
		var setSize = function() {
			...
			for(var F=0;F<u.length;F++){
				...
				var q = $(u[F]),
					K = q[0],
					A = parseInt(q.outerWidth(),10),
					I = parseInt(q.outerHeight(),10);
				if(!A) {A = parseInt(K.offsetWidth,10)}
				if(!I) {I = parseInt(K.offsetHeight,10)}
				v = A>v ? A : v;
				B = I>B ? I : B
			}
			if(v>0 && B>0){
		...
		var func = setInterval(function() {
			...
			var flag = true;
			$("img",J).each(function() { if(!this.complete){ flag=false; } });
			if(!flag && cnt<5000) {
				cnt++;
				return;
			}
		...
		J.each(function(){
			...
			if(N.is("img")){
				var L=( /*$.browser.msie*/navigator.userAgent.match(/MSIE/) && this.cycleW==28 && this.cycleH==30 && !this.complete );
				var O=(/*$.browser.mozilla*/navigator.userAgent.match(/Firefox/) && this.cycleW==34 && this.cycleH==19 && !this.complete);
				var M=(/*$.browser.opera*/navigator.userAgent.match(/Opera/) && ((this.cycleW==42&&this.cycleH==19)||(this.cycleW==37 && this.cycleH==17)) && !this.complete);
				...
				if(L||O||M||w){
					if(E.s&&C.requeueOnImageNotLoaded&&++t.requeueAttempts<100){
						...
						setTimeout(function(){$(E.s,E.c).cycle(t)},C.requeueTimeout);
						s=true;
						return false
					}else{
		...
		if(C.timeout){
			...
			if(C.speed.constructor==String){
				C.speed=$.fx.speeds[C.speed]||parseInt(C.speed)
			}
			if(!C.sync){
				C.speed=C.speed/2
			}
		...
		C.slideCount=u.length;C.currSlide=C.lastSlide=x;
		if(C.random){
		...
		if(!C.multiFx){
			var G=$.fn.cycle.transitions[C.fx];
			if($.isFunction(G)){
				G(y,J,C)
			}else{
		...
		if(C.next){
			$(C.next).bind(C.prevNextEvent,function(){return o(C,C.rev?-1:1)})
		}
		if(C.prev){
			$(C.prev).bind(C.prevNextEvent,function(){return o(C,C.rev?1:-1)})
		}
		if(C.pager){d(u,C)}
		j(C,u);
		return C
	...
	function n(q){
		q.original={before:[],after:[]};
		q.original.cssBefore=$.extend({},q.cssBefore);
		q.original.cssAfter=$.extend({},q.cssAfter);
		q.original.animIn=$.extend({},q.animIn);
		q.original.animOut=$.extend({},q.animOut);
		$.each(q.before,function(){
			q.original.before.push(this)
		});
		$.each(q.after,function(){
			q.original.after.push(this)
		})
	}
	function c(w){
		var u,s,r=$.fn.cycle.transitions;
		if(w.fx.indexOf(",")>0){
			w.multiFx=true;
			...
			for(u=0;u<w.fxs.length;u++){
				var v=w.fxs[u];
				s=r[v];
				if(!s||!r.hasOwnProperty(v)||!$.isFunction(s)){
			...
			if(w.fx=="all"){
				...
				for(p in r){
					s=r[p];
					if(r.hasOwnProperty(p)&&$.isFunction(s)){
						w.fxs.push(p)
					}
				}
			}
	...
	function j(r,q){
		r.addSlide=function(u,v){
			var t=$(u),w=t[0];
			if(!r.autostopCount){r.countdown++}
			q[v?"unshift":"push"](w);
			if(r.els){
			...
			if(!$.support.opacity&&r.cleartype&&!r.cleartypeNoBg){g(t)}
			if(r.fit&&r.width){t.width(r.width)}
			if(r.fit&&r.height&&r.height!="auto"){$slides.height(r.height)}
			w.cycleH=(r.fit&&r.height)?r.height:t.height();
			w.cycleW=(r.fit&&r.width)?r.width:t.width();
			t.css(r.cssBefore);
			if(r.pager){
				$.fn.cycle.createPagerAnchor(q.length-1,w,$(r.pager),q,r)
			}
			if($.isFunction(r.onAddSlide)){
				r.onAddSlide(t)
			}else{
				t.hide()
			}
	...
	$.fn.cycle.resetState=function(r,q){
		q=q||r.fx;
		r.before=[];r.after=[];
		r.cssBefore=$.extend({},r.original.cssBefore);
		r.cssAfter=$.extend({},r.original.cssAfter);
		r.animIn=$.extend({},r.original.animIn);
		r.animOut=$.extend({},r.original.animOut);
		r.fxFn=null;
		$.each(r.original.before,function(){
			r.before.push(this)
		});
		$.each(r.original.after,function(){
			r.after.push(this)
		});
		var s=$.fn.cycle.transitions[q];
		if($.isFunction(s)){
			s(r.$cont,$(r.elements),r)
		}
	};
	function e(x,q,w,y){
		...
		if(w||!u.cyclePause){
			var v=q.fx;
			A.cycleH=A.cycleH||$(A).height();
			A.cycleW=A.cycleW||$(A).width();
			z.cycleH=z.cycleH||$(z).height();
			z.cycleW=z.cycleW||$(z).width();
			if(q.multiFx){
				if(q.lastFx==undefined||++q.lastFx>=q.fxs.length){q.lastFx=0}
				v=q.fxs[q.lastFx];
				q.currFx=v
			}
			if(q.oneTimeFx){
			...
			$.fn.cycle.resetState(q,v);
			if(q.before.length){
				$.each(q.before,function(B,C){
					if(u.cycleStop!=q.stopCount){return}
					C.apply(z,[A,z,q,y])
				})
			}
			var s=function(){
				$.each(q.after,function(B,C){
					if(u.cycleStop!=q.stopCount){return}
					C.apply(z,[A,z,q,y])
				})
			};
			if(q.nextSlide!=q.currSlide){
				...
				if(q.fxFn){
					...
					if($.isFunction($.fn.cycle[q.fx])){
						$.fn.cycle[q.fx](A,z,q,s)
					}else{
						$.fn.cycle.custom(A,z,q,s,w&&q.fastOnEvent)
					}
				}
			}
			...
			if(q.pager){
				$.fn.cycle.updateActivePagerLink(q.pager,q.currSlide)
			}
		}
		var r=0;
	...
	$.fn.cycle.updateActivePagerLink=function(q,r){
		$(q).each(function(){
			$(this).find("a").removeClass("activeSlide").filter("a:eq("+r+")").addClass("activeSlide")
		})
	};
	function h(v,s,u,r){
		if(u.timeoutFn){
			var q=u.timeoutFn(v,s,u,r);
			while((q-u.speed)<250){q+=u.speed}
			a("calculated timeout: "+q+"; speed: "+u.speed);
			if(q!==false){return q}
		}
		return u.timeout
	}
	$.fn.cycle.next=function(q){
		o(q,q.rev?-1:1)
	};
	$.fn.cycle.prev=function(q){
		o(q,q.rev?1:-1)
	};
	function o(r,u){
		var q=r.elements;
		...
		if(r.random&&u<0){
			...
			if(r.random){
				if(++r.randomIndex==q.length){r.randomIndex=0}
				r.nextSlide=r.randomMap[r.randomIndex]
			}else{
				r.nextSlide=r.currSlide+u;
				if(r.nextSlide<0){
					if(r.nowrap){return false}
	...
	function d(r,s){
		var q=$(s.pager);
		$.each(r,function(t,u){
			$.fn.cycle.createPagerAnchor(t,u,q,r,s)
		});
		$.fn.cycle.updateActivePagerLink(s.pager,s.startingSlide)
	}
	$.fn.cycle.createPagerAnchor=function(u,v,s,t,w){
		var r;
		if($.isFunction(w.pagerAnchorBuilder)){
	...
	$.fn.cycle.hopsFromLast=function(t,s){
	...
	function g(s){
		...
		function q(w){
			for(;w&&w.nodeName.toLowerCase()!="html";w=w.parentNode){
				var t=$.css(w,"background-color");
				if(t.indexOf("rgb")>=0){
					var u=t.match(/\d+/g);
					return"#"+r(u[0])+r(u[1])+r(u[2])
				}
				if(t&&t!="transparent"){return t}
			}
			return"#ffffff"
		}
		s.each(function(){
			$(this).css("background-color",q(this))
		})
	}
	$.fn.cycle.commonReset=function(v,t,u,r,s,q){
		$(u.elements).not(v).hide();
		...
		$(v).css("zIndex",u.slideCount+(q===true?1:0));
		$(t).css("zIndex",u.slideCount+(q===true?0:1))
	};
	$.fn.cycle.custom=function(B,v,q,s,r){
		var A=$(B),w=$(v);
	...
	$.fn.cycle.transitions={
		fade:function(r,s,q){
			...
			q.before.push(function(v,t,u){
				$.fn.cycle.commonReset(v,t,u);
				u.cssBefore.opacity=0
			});
			q.animIn={opacity:1};
			q.animOut={opacity:0};
			q.cssBefore={top:0,left:0}
		}
	};
...
(function(a){
	...
	a.fn.cycle.transitions.zoom=function(c,d,b){
		b.before.push(function(g,e,f){
			...
			f.cssBefore.top=e.cycleH/2;f.cssBefore.left=e.cycleW/2;
	...
	a.fn.cycle.transitions.blindZ=function(e,f,d){
		...
		d.before.push(function(i,g,h){
			...
			h.animIn.height=g.cycleH;h.animOut.top=$.cycleH
		});
